...
export default function Connections() {
  ...
  const emptyConfig = activeTab === 'followers'
    ? { icon: 'people-outline' as const, title: 'Aún no tiene seguidores', sub: username ? `@${username} no tiene seguidores todavía` : 'Sin seguidores' }
    : { icon: 'person-add-outline' as const, title: 'Aún no sigue a nadie', sub: username ? `@${username} no sigue a nadie todavía` : 'Sin seguidos' };
    ...
    <View style={styles.container}>
      ...
      {currentLoading ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[styles.listContent, currentList.length === 0 && { flexGrow: 1, justifyContent: 'center' }]}
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={[styles.userRow, isWeb && { maxWidth: 800, alignSelf: 'center', width: '100%' }]}>
                <TouchableOpacity style={styles.userInfo} onPress={() => goToProfile(item.username, item.uid)} activeOpacity={0.7}>
                  <Avatar photoURL={item.photoURL} fallback={item.username} size={48} />
                  <View style={styles.userText}>
                    <View style={styles.usernameRow}>
                      <Text style={styles.username} numberOfLines={1}>@{item.username}</Text>
                      {isMe && <View style={styles.meBadge}><Text style={styles.meBadgeText}>Tú</Text></View>}
                    </View>
                    {item.bio ? <Text style={styles.bio} numberOfLines={1}>{item.bio}</Text> : <Text style={styles.noBio}>Sin biografía</Text>}
                  </View>
                </TouchableOpacity>
                {!isMe && <FollowButton targetUid={item.uid} targetUsername={item.username} size="small" />}
              </View>
            );
          }}
        />
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 12, gap: 8 },
  iconBtn: { padding: 4 },
  headerCenter: { flex: 1, alignItems: 'center' },
  headerTitle: { color: '#fff', fontSize: 16, fontWeight: '700', maxWidth: 240 },
  tabsContainer: { flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#1e1e1e', position: 'relative' },
  tab: { flex: 1, paddingVertical: 14, alignItems: 'center' },
  tabText: { color: '#666', fontSize: 14, fontWeight: '600' },
  tabTextActive: { color: '#fff' },
  tabCount: { fontWeight: '800' },
  tabIndicator: { position: 'absolute', bottom: 0, width: '50%', height: 2.5, backgroundColor: '#208c8c', borderTopLeftRadius: 2, borderTopRightRadius: 2 },
  ...
  meBadge: { backgroundColor: '#222', paddingHorizontal: 7, paddingVertical: 2, borderRadius: 6, borderWidth: 1, borderColor: '#2e2e2e' },
